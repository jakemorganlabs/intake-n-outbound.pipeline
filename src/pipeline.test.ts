...
describe('Pipeline', () => {
  ...
  describe('router', () => {
    ...
    it('routes 40-69 as WARM', () => {
      expect(router({ composite: 50, confidence: 0.8 }).tier).toBe('WARM');
    });

    it('routes <40 as COLD', () => {
      expect(router({ composite: 30, confidence: 0.9 }).tier).toBe('COLD');
    });

    it('always routes inference_failed as MANUAL', () => {
      expect(router({ composite: 96, confidence: 0.86, inference_failed: true }).tier).toBe('MANUAL');
    });
  });
