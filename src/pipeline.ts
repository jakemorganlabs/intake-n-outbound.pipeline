...
async function dispatchAdapters(
  ...
  if (routing.tier === 'HOT') {
    ...
    results.push({ ok: chatResult.ok, adapter: 'chat', latencyMs: chatResult.latencyMs, error: chatResult.error });

    const crmResult = await dispatchCRM({
      leadId,
      idempotencyKey,
      name: normalized.name,
      email: normalized.email,
  ...
  if (routing.tier === 'WARM') {
    ...
    results.push({ ok: sheetResult.ok, adapter: 'sheet', latencyMs: sheetResult.latencyMs, error: sheetResult.error });
  }

  // COLD and MANUAL: no outbound action
  return results;
}
...
export async function runPipeline(payload: PipelineInput, overrides?: PipelineOverrides): Promise<PipelineOutput> {
  ...
  if (inferenceResult.error) {
    ...
    await writeDeadLetter({
      leadSnapshot: {
        ...
        normalized: { name: normalized.name, email: normalized.email, domain: normalized.domain, company: normalized.company, message: normalized.message },
        routing,
        inference_error: inferenceResult.error,
      },
      stage: 'inference',
    ...
    try {
      await dispatchChatAlert({
        leadId,
        tier: 'MANUAL',
        summary: `Inference failed for ${normalized.name} (${normalized.email}): ${inferenceResult.error}`,
        composite: 0,
        confidence: 0,
        name: normalized.name,
        email: normalized.email,
  ...
  if (status === 'routed' && enrichment && score) {
    dispatchResults = await dispatchAdapters(
      leadId, idempotencyKey, normalized, enrichment, score, routing,
      inferenceResult.result, validation, status
    );
  }
  mark('dispatch');
  ...
  if (failed.length > 0) {
    ...
    for (const f of failed) {
      await writeDeadLetter({
        leadSnapshot: {
          ...
          routing,
          score,
          failed_adapter: f.adapter,
          dispatch_error: f.error,
        },
        stage: `outbound_${f.adapter}`,
        error: f.error ?? `Outbound adapter ${f.adapter} failed`,
    ...
    try {
      await dispatchChatAlert({
        leadId,
        tier: routing.tier,
        summary: `${failed.length} outbound adapter(s) failed for ${normalized.name}: ${failed.map(f => f.adapter).join(', ')}`,
  ...
  if (dispatchResults.length > 0) {
    responseBody.dispatch = dispatchResults.map(r => ({ adapter: r.adapter, ok: r.ok, latency_ms: r.latencyMs, error: r.error }));
  }
